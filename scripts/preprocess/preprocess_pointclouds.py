'''
HOW TO RUN:
python scripts\preprocess_pointclouds.py --dataset_root data\Shapenetcore_benchmark --output_dir data\processed_pointcloud
'''
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Preprocess Kaggle ShapeNet point clouds into 32^3 voxel tensors.")
    parser.add_argument("--dataset_root", type=str, required=True, help="Root folder of ShapeNetcore_benchmark")
    parser.add_argument("--output_dir", type=str, required=True, help="Output folder for voxel .pt files and index.jsonl")
    parser.add_argument("--resolution", type=int, default=32, help="Voxel grid resolution")
    parser.add_argument("--splits_dir", type=str, default=None, help="Directory containing train_split.json / val_split.json / test_split.json")
...
def main() -> None:
    ...
    if args.splits_dir is not None:
        ...
        splits_dir = dataset_root / "train_test_split"

    voxel_dir.mkdir(parents=True, exist_ok=True)

    split_files = {
        "train": splits_dir / "train_split.json",
        "val": splits_dir / "val_split.json",
        "test": splits_dir / "test_split.json",
    }
